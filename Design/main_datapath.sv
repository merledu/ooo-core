module main_datapath #(
    ...
    // Frontend Stall Control
    logic id_stall_frontend, rn_stall_frontend, dis_stall_frontend, stall_frontend;
    assign stall_frontend = id_stall_frontend | rn_stall_frontend | dis_stall_frontend;
    
    // Global & Flush Signals
    logic flush, rob_global_flush;
    logic [XLEN-3:0] rob_flush_pc;

    // CDB & Arbiter Signals (Internalized)
    logic stall_issue;
    logic cdb_done1, cdb_done2, cdb_wakeup1, cdb_wakeup2; 
    ...
    logic restore_ghr, restore_ras, update_btb, update_ras; 
    ...
    logic lsq_mem_write, lsq_mem_read, lsq_mem_sign_ext;
    ...
    // Functional Unit Acks (Internalized)
    logic mul_ack, div_ack;
    logic cdb_mul_busy, cdb_div_busy;
    
    // BIQ Signals
    logic dis_biq_dealloc;
    logic [BIQ_ADDRESS-1:0] rr_biq_id;
    logic rr_slot_id;
    logic biq_pred_taken1, biq_pred_taken2;
    logic [XLEN-1:0] biq_pred_target1, biq_pred_target2;
    ...
    DIS_Stage #(
        ...
        .rn_rob_index1           (rn_rob_index1), .rn_rob_index2(rn_rob_index2),
        
        .cdb_branch_rob_index    (cdb_branch_rob_index),
        .cdb_address_available1  (cdb_address_available1), .cdb_is_load1(cdb_is_load1),
        ...
        .cdb_result1_available   (cdb_result1_available), .cdb_result2_available(cdb_result2_available),
        .cdb_result1_broadcast   (cdb_result1_broadcast), .cdb_result2_broadcast(cdb_result2_broadcast),
    ...
    EX_Stage #(
        ...
        .mul_ack                 (mul_ack), 
        .div_ack                 (div_ack),
        .biq_pred_taken1         (biq_pred_taken1), .biq_pred_taken2(biq_pred_taken2),
        .biq_pred_target1        (biq_pred_target1), .biq_pred_target2(biq_pred_target2),
        .rr_instr1_read_data1    (rr_instr1_read_data1), .rr_instr1_read_data2(rr_instr1_read_data2),
        .rr_instr2_read_data1    (rr_instr2_read_data1), .rr_instr2_read_data2(rr_instr2_read_data2),
        .rr_instr1_prs1          (rr_instr1_prs1), .rr_instr1_prs2(rr_instr1_prs2),
        .rr_instr2_prs1          (rr_instr2_prs1), .rr_instr2_prs2(rr_instr2_prs2),
        ...
        .rr_instr1_regsrc1       (rr_instr1_regsrc1), .rr_instr1_regsrc2(rr_instr1_regsrc2), .rr_isimm1(rr_isimm1), .rr_retaddr1(rr_retaddr1), .rr_upperimm1(rr_upperimm1),
        .rr_regwrite1            (rr_regwrite1), .rr_memwrite1(rr_memwrite1), .rr_memtoreg1(rr_memtoreg1),
        .rr_pc1                  (rr_pc1), .rr_prd1(rr_prd1), .rr_immediate1(rr_immediate1), .rr_alu_operation1(rr_alu_operation1),
        .rr_branch_tag1          (rr_branch_tag1), .rr_branch_mask1(rr_branch_mask1), .rr_biq_address1(rr_biq_address1), .rr_rob_index1(rr_rob_index1),
        
        .rr_valid2               (rr_valid2), .rr_is_m_extension2(rr_is_m_extension2), .rr_jump_reg2(rr_jump_reg2), .rr_jump2(rr_jump2), .rr_branch2(rr_branch2),
        .rr_instr2_regsrc1       (rr_instr2_regsrc1), .rr_instr2_regsrc2(rr_instr2_regsrc2), .rr_isimm2(rr_isimm2), .rr_retaddr2(rr_retaddr2), .rr_upperimm2(rr_upperimm2),
        .rr_regwrite2            (rr_regwrite2), .rr_memwrite2(rr_memwrite2), .rr_memtoreg2(rr_memtoreg2),
        .rr_pc2                  (rr_pc2), .rr_prd2(rr_prd2), .rr_immediate2(rr_immediate2), .rr_alu_operation2(rr_alu_operation2),
        .rr_branch_tag2          (rr_branch_tag2), .rr_branch_mask2(rr_branch_mask2), .rr_biq_address2(rr_biq_address2), .rr_rob_index2(rr_rob_index2),
        ...
        .ex_alu1_valid           (ex_alu1_valid), .ex_alu1_regwrite(ex_alu1_regwrite), .ex_alu1_result(ex_alu1_result), .ex_alu1_prd(ex_alu1_prd), .ex_alu1_rob_index(ex_alu1_rob_index),
        .ex_alu2_valid           (ex_alu2_valid), .ex_alu2_regwrite(ex_alu2_regwrite), .ex_alu2_result(ex_alu2_result), .ex_alu2_prd(ex_alu2_prd), .ex_alu2_rob_index(ex_alu2_rob_index),
        .ex_mul_valid            (ex_mul_valid), .ex_mul_result(ex_mul_result), .ex_mul_prd(ex_mul_prd), .ex_mul_rob_index(ex_mul_rob_index),
        .cdb_mul_busy            (cdb_mul_busy),
        .ex_div_valid            (ex_div_valid), .ex_div_result(ex_div_result), .ex_div_prd(ex_div_prd), .ex_div_rob_index(ex_div_rob_index),
        .cdb_div_busy            (cdb_div_busy),
